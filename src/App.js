...
function App() {
  ...
  const makeGroup = (e) => {
    ...
    const groupRoute = groupRouteRef.current.value
      .split(",")
      .filter((e) => e !== "");
    try {
  ...
  const makeEntry = (e) => {
    ...
    const entryRoute = entryRouteRef.current.value
      .split(",")
      .filter((e) => e !== "");
    try {
  ...
  const changeEntry = (e) => {
    e.preventDefault();
    changeEntryData(
      modifyRouteRef.current.value.split(","),
      JSON.parse(modifyDataRef.current.value)
    );
  };
  ...
  const modifyNameEntry = (e) => {
    e.preventDefault();
    renameEntry(
      renameRouteRef.current.value.split(","),
      renameNameRef.current.value
    );
  };
  ...
  const updateEntrySelection = (entryName, route) => {
    ...
    modifyDataRef.current.value = JSON.stringify(
      getEntry([...route.split(","), entryName]).data,
      null,
      2
    );
  };
  ...
  const renderEntry = (entry, route = "") => {
    return (
      <ul key={`${route},${entry.name}`}>
        <b onClick={() => updateEntrySelection(entry.name, route)}>
          {entry.name}
        </b>
        {Object.entries(entry.data).map(([key, value]) => (
          <li key={`${route},${entry.name}:${key}`}>
            {key}: {value}
          </li>
        ))}
      </ul>
  ...
  return (
    <div style={{ display: "flex" }}>
      ...
      <div>{nestedList.map((group) => renderGroup(group))}</div>
    </div>
  );
}
